...
public class Move2D : MonoBehaviour
{
    ...
    private void OnMove(InputValue inputVec) //takes a input value and checks when pressed, how long it held, and when its released, and changes certain values based on that
    { //takes in Vector2
        ...
        float result;
        result = inputVec.Get<float>();

        if (result < 0)
        {
            Debug.Log(result +" , buttonPushed");
        }
    ...
    private void FixedUpdate()
    {
        ...
        //movementVec = new Vector3(speedX, 0, speedY);
